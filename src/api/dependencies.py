...
def _init_reranker(config: AppConfig):
    ...
    if not config.rerank.enabled:
        return None
    rerank_key = os.getenv(
        "RERANK_API_KEY", os.getenv("EMBEDDING_API_KEY", os.getenv("LLM_API_KEY", ""))
    )
    ...
    try:
        ...
        if provider == "deepseek":
            llm = OpenAICompatibleProvider(
                api_key=api_key, base_url="https://api.deepseek.com/v1", model=config.llm.model
            )
        elif provider == "openai":
            llm = OpenAICompatibleProvider(
                api_key=api_key, base_url="https://api.openai.com/v1", model=config.llm.model
            )
        elif provider == "ollama":
            llm = OpenAICompatibleProvider(
                api_key="ollama", base_url="http://localhost:11434/v1", model=config.llm.model
            )
        else:
